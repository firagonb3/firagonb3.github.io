<?php
    function PSN($cadena) {
        if (preg_match('@Playstation@', $cadena) or preg_match('@PSN@', $cadena) or preg_match('@PSN@', $cadena) or preg_match('@PSX@', $cadena) or preg_match('@PS4@', $cadena) or preg_match('@PS5@', $cadena)) {
            return true;
        }
        return false;
    }
    ...
    function feed($feedURL, $IMG){
        ...
        foreach($rss->channel->item as $item) { 
            ...
            $description = strip_tags($item->description);  //extrae la descriPSNion
            $description2 = strip_tags($item->description);  //extrae la descriPSNion

            if (strlen($description) > 100) { //limita la descriPSNion a 200 caracteres
            ...
            if (PSN($title) or PSN($description2)) {
                echo "
                    <div class='col'  id='". $i ."'>
