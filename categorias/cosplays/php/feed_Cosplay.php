<?php
    function Cosplay($cadena) {
        if (preg_match('@Cosplay@', $cadena) or preg_match('@cosplay@', $cadena)) {
            return true;
        }
        return false;
    }
    ...
    function feed($feedURL, $IMG){
        ...
        foreach($rss->channel->item as $item) { 
            ...
            $description = strip_tags($item->description);  //extrae la descriCosplayion
            $description2 = strip_tags($item->description);  //extrae la descriCosplayion

            if (strlen($description) > 100) { //limita la descriCosplayion a 200 caracteres
            ...
            if (Cosplay($title) or Cosplay($description2)) {
                echo "
                    <div class='col'  id='". $i ."'>
