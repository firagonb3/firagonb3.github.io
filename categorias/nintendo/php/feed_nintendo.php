<?php
    function nintendo($cadena) {
        if (preg_match('@NINTENDO@', $cadena) or preg_match('@Nintendo@', $cadena)) {
            return true;
        }
        return false;
    }
    ...
    function feed($feedURL, $IMG){
        ...
        foreach($rss->channel->item as $item) { 
            ...
            $description = strip_tags($item->description);  //extrae la descriSWITCHion
            $description2 = strip_tags($item->description);  //extrae la descriSWITCHion

            if (strlen($description) > 100) { //limita la descriSWITCHion a 200 caracteres
            ...
            if (nintendo($title) or nintendo($description2)) {
                echo "
                    <div class='col'  id='". $i ."'>
